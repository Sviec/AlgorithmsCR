...
void dfs_algorithm(const vector<vector<int>>& graph, int v, vector<bool>& visited, vector<int>& component) {
    visited[v] = true;
    component.push_back(v + 1); 
    for (int neighbor : graph[v]) {
        if (!visited[neighbor]) {
            dfs_algorithm(graph, neighbor, visited, component);
        }
    }
}
...
void dfs()
{
    ...
    dfs_algorithm(graph, 0, visited, component);
